...
class StreamTest(unittest.TestCase):
    ...
    def test_empty(self):
        cb = mock.Mock()
        eachpng.handle_stream(StringIO(), cb)
        cb.assert_not_called()


    def test_no_png(self):
        cb = mock.Mock()
        eachpng.handle_stream(StringIO(eachpng.PNG_START), cb)
        cb.assert_not_called()


    def test_single_png(self):
        cb = mock.Mock()
        eachpng.handle_stream(StringIO('pre' + eachpng.PNG_START + 'someContent' + eachpng.PNG_END + 'post'), cb)
        cb.assert_called_once_with(eachpng.PNG_START + 'someContent' + eachpng.PNG_END)

    def test_multiple_png(self):
        cb = mock.Mock()
        eachpng.handle_stream(StringIO(
